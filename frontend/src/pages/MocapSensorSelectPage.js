{/*
    Model preview page that provides preview of the motion sequence generated from motion capture data
    Similar to SensorSelectPage.js
    Load data from MocapPage instead of ModelProcessPage
    @Author: Liang Xu
* */}
...
import {totalVertex_mocap, num_frame_mocap, totalJoint_mocap, folder_id_mocap} from "./MocapPage";
...
function BufferPoints({setPoint, nframe, setNframe, setPlay, play, checked, setFace, forceUp}) {
    ...
    const p = new Float32Array(totalVertex_mocap[0].data);
    ...
    useFrame(() => {
        if (nframe === 0){
            const p = new Float32Array(totalVertex_mocap[nframe].data);
            const points = new BufferAttribute(p, 3);
        ...
        if (play) {
            if (nframe === totalVertex_mocap.length-1){
                setPlay(false)
            }
...
function Test() {
    ...
    const clickEvent = (setPoint, nframe, checked, setFace, forceUp) => {
        ...
        point = new Float32Array(totalVertex_mocap[n].data);
        min = Infinity
        if (checked === true) {
...
function Point({ pt, nframe }) {
    ...
    return (
        <mesh position={[totalVertex_mocap[n].data[pt*3], totalVertex_mocap[n].data[pt*3+1], totalVertex_mocap[n].data[pt*3+2]]} >
            <sphereBufferGeometry attach="geometry" args={[0.005, 16, 16]}/>
            <meshNormalMaterial attach="material"/>
        </mesh>
    );
...
function Joint({ pt, nframe }) {
    ...
    return (
        <mesh position={[totalJoint_mocap[n].data[pt * 3], totalJoint_mocap[n].data[pt * 3 + 1], totalJoint_mocap[n].data[pt * 3 + 2]]}>
...
function Face({ fs, nframe }) {
    ...
    const p = new Float32Array([totalVertex_mocap[n].data[fs*9], totalVertex_mocap[n].data[fs*9+1], totalVertex_mocap[n].data[fs*9+2],
                                        totalVertex_mocap[n].data[fs*9+3], totalVertex_mocap[n].data[fs*9+4], totalVertex_mocap[n].data[fs*9+5],
                                        totalVertex_mocap[n].data[fs*9+6], totalVertex_mocap[n].data[fs*9+7], totalVertex_mocap[n].data[fs*9+8]]);
    console.log(fs)
    return (
        <mesh position={[0,0,0]}>
            <bufferGeometry attach="geometry">
...
function Face1({ fs, nframe }) {
    ...
    console.log(fs)
    return (
        <mesh position={[totalVertex_mocap[n].data[fs*9], totalVertex_mocap[n].data[fs*9+1], totalVertex_mocap[n].data[fs*9+2]]} >
            <sphereBufferGeometry attach="geometry" args={[0.005, 16, 16]}/>
...
function Face2({ fs, nframe }) {
    ...
    console.log(fs)
    return (
        <mesh position={[totalVertex_mocap[n].data[fs*9+3], totalVertex_mocap[n].data[fs*9+4], totalVertex_mocap[n].data[fs*9+5]]} >
            <sphereBufferGeometry attach="geometry" args={[0.005, 16, 16]}/>
            <meshBasicMaterial color="red" attach="material" />
        </mesh>
    );
...
function Face3({ fs, nframe }) {
    ...
    console.log(fs)
    return (
        <mesh position={[totalVertex_mocap[n].data[fs*9+6], totalVertex_mocap[n].data[fs*9+7], totalVertex_mocap[n].data[fs*9+8]]} >
            <sphereBufferGeometry attach="geometry" args={[0.005, 16, 16]}/>
            <meshBasicMaterial color="red" attach="material" />
        </mesh>
    );
...
function PlayEvent() {
    ...
    const clickEvent = (setPlay, play, setNframe, nframe) => {
        if (nframe === totalVertex_mocap.length && play === false){
            setNframe(0);
        }
        setPlay(!play)
    }
...
export default function MocapSensorSelectPage({pageChange}) {
    ...
    if (totalVertex_mocap.length !== n ){
        setN(totalVertex_mocap.length)
    }
    ...
    const completeSelect = (index) => {
        ...
        let tmp_p_1 = -1
        let tmp_p_2 = -1
        let point = totalVertex_mocap[0].data
    ...
    const handleConfirm = async () => {
        ...
        for (let id = 0; id < faces.length; id++){
            ...
            console.log(selectT)
            let result = ""
            for (let i = 0; i < totalVertex_mocap.length; i++) {
                for (let j = 0; j < selectT.length; j++) {
                    let point = totalVertex_mocap[i].data
            ...
            const data = {
                ...
                'folder_id': folder_id_mocap
            };
            const response = await axios.post('http://'+addr+'/mocap/confirmSelect', data);
        ...
        <div style={{width: "100%", height: "90%", paddingTop: "10%"}}>
            ...
            <div className={"video-slider"}>
                <div className={"video-slider-up"}>
                    ...
                    <div>
                        <div>{nframe} / {num_frame_mocap}</div>
                    </div>
                </div>
                <div>
                    <Slider
